...
class PixelsDiceMachine(PixelsDiceMachineInterface):
    ...
    def __init__(
        ...
        elif not isinstance(connectionLoopSleepTimeSeconds, (float, int)):
        ...
        elif not isinstance(eventLoopSleepTimeSeconds, (float, int)):
            raise TypeError(f'eventLoopSleepTimeSeconds argument is malformed: \"{eventLoopSleepTimeSeconds}\"')
        elif eventLoopSleepTimeSeconds < 0.125 or eventLoopSleepTimeSeconds > 3:
            raise ValueError(f'eventLoopSleepTimeSeconds argument is out of bounds: {eventLoopSleepTimeSeconds}')
        elif not isinstance(queueTimeoutSeconds, int):
            raise TypeError(f'queueTimeoutSeconds argument is malformed: \"{queueTimeoutSeconds}\"')
        elif queueTimeoutSeconds < 1 or queueTimeoutSeconds > 5:
            raise ValueError(f'queueTimeoutSeconds argument is out of bounds: {queueTimeoutSeconds}')
        elif not isinstance(notifyCharacteristicUuid, str):
            raise TypeError(f'notifyCharacteristicUuid argument is malformed: \"{notifyCharacteristicUuid}\"')
        elif not isinstance(pixelsDiceName, str):
            raise TypeError(f'pixelsDiceName argument is malformed: \"{pixelsDiceName}\"')
        ...
        self.__pixelsDiceName: Final[str] = pixelsDiceName
    ...
    async def __connectToDice(self) -> DiceBluetoothInfo | None:
        ...
        for device, advertisement in devices.values():
            ...
            if diceDevice is None and device.name.casefold() == self.__pixelsDiceName.casefold():
                diceDevice = device
